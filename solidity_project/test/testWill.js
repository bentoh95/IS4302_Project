...
const contractABIPath = path.resolve(__dirname, "../artifacts/contracts/Will.sol/Will.json");
...
describe("Will", function() {
  let Will;
  let will;
  let owner, beneficiary1, beneficiary2, editor1, editor2, viewer1, viewer2, nonOwner, residualBeneficiary1, residualBeneficiary2, others;

  // Deploy Will
  beforeEach(async function() {
      [owner, beneficiary1, beneficiary2, editor1, editor2, viewer1, viewer2, nonOwner, residualBeneficiary1, residualBeneficiary2, ...others] = await ethers.getSigners();
  ...
  it("Should add to residual beneficiary successfully", async function () {
      ...
      const allocation1 = await will.getBeneficiaryAllocationPercentage(owner.address, beneficiary1.address);
      const allocation2 = await will.getBeneficiaryAllocationPercentage(owner.address, beneficiary2.address);
      const allocation3 = await will.getBeneficiaryAllocationPercentage(owner.address, residualBeneficiary1.address);
      expect(allocation1).to.equal(20);
  ...
  it("Should not exceed 100% after adding beneficiary", async function () {
      ...
      await expect(will.addBeneficiaries(owner.address, beneficiaries, allocations)).to.be.revertedWith("Total allocation exceeds 100%");
  });
  ...
  it("Should not update the allocation for a beneficiary if it exceeds 100 (w/o residual)", async function () { 
      ...
      //bene1, bene2
      //40, 60
      await will.setResidualBeneficiary(owner.address,residualBeneficiary2.address);
      await expect(will.updateAllocations(owner.address, beneficiaries, updatedAllocations)).to.be.revertedWith("Total allocation exceeds 100%");
      //bene1, bene2
      //20, 100
  });
  ...
  it("Should revert if removing residual beneficiary", async function () {
      ...
      await will.addBeneficiaries(owner.address, [beneficiary1.address, beneficiary2.address], [30, 40]);
  ...
  it("Should remove beneficiary and new residuary gets remaining", async function () { 
      ...
      const allocation2 = await will.getBeneficiaryAllocationPercentage(owner.address, beneficiary2.address);
      const allocation3 = await will.getBeneficiaryAllocationPercentage(owner.address, residualBeneficiary1.address);
      const allocation4 = await will.getBeneficiaryAllocationPercentage(owner.address, residualBeneficiary2.address);
      console.log(allocation1);
  ...
  it("Should return correct will details", async function () {
      ...
      // Construct the expected string based on your logic
      const expectedWill = `Beneficiaries & Allocations:\n- ${beneficiary1.address.toLowerCase().substring(2)} -> 70\n- ${beneficiary2.address.toLowerCase().substring(2)} -> 30\nTotal Digital Assets: 100\n`;
  ...
  it("Should not allow unauthorised editor to add beneficiaries", async function () {
      ...
      await expect(will.connect(nonOwner).addBeneficiaries(owner.address, beneficiaries, initialAllocations)).to.be.revertedWith("Not authorized");
  });
  ...
  it("Should allow authorised editor to add beneficiaries", async function () {
      ...
      const beneficiaries = [beneficiary1.address, beneficiary2.address];
      const initialAllocations = [30, 70];
      await will.connect(editor1).addBeneficiaries(owner.address, beneficiaries, initialAllocations);
      const allocation1 = await will.getBeneficiaryAllocationPercentage(owner.address, beneficiary1.address);
      const allocation2 = await will.getBeneficiaryAllocationPercentage(owner.address, beneficiary2.address);
      expect(allocation1).to.equal(30); 
      expect(allocation2).to.equal(70); 
  ...
  it("Should not allow unauthorised editor to remove beneficiaries", async function () {
      ...
      const beneficiariesToRemove = [beneficiary1.address];
      await will.addBeneficiaries(owner.address, beneficiaries, initialAllocations);
      await expect(will.connect(nonOwner).removeBeneficiaries(owner.address, beneficiariesToRemove)).to.be.revertedWith("Not authorized");
  });
  ...
  it("Should allow authorised editor to remove beneficiaries", async function () {
      ...
      const beneficiariesToRemove = [beneficiary1.address];
      await will.addBeneficiaries(owner.address, beneficiaries, initialAllocations);
      await will.connect(editor1).removeBeneficiaries(owner.address, beneficiariesToRemove);
      const allocation1 = await will.getBeneficiaryAllocationPercentage(owner.address, beneficiary2.address);
      const allocation2 = await will.getBeneficiaryAllocationPercentage(owner.address, residualBeneficiary1.address);
      const allocation3 = await will.getBeneficiaryAllocationPercentage(owner.address, beneficiary1.address);
  ...
  it("Should not allow unauthorised editor to update allocations", async function () {
      ...
      await will.addBeneficiaries(owner.address, beneficiaries, initialAllocations);
      await expect(will.connect(nonOwner).updateAllocations(owner.address, beneficiaries, updatedAllocations)).to.be.revertedWith("Not authorized");
  });
  ...
  it("Should allow authorised editor to update allocations", async function () {
      ...
      const updatedAllocations = [20, 80];
      await will.addBeneficiaries(owner.address, beneficiaries, initialAllocations);
      await will.connect(editor1).updateAllocations(owner.address, beneficiaries, updatedAllocations);
      const allocation1 = await will.getBeneficiaryAllocationPercentage(owner.address, beneficiary1.address);
      const allocation2 = await will.getBeneficiaryAllocationPercentage(owner.address, beneficiary2.address);
      expect(allocation1).to.equal(20); 
      expect(allocation2).to.equal(80); 
